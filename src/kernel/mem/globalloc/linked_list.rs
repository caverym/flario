...
impl ListNode {
    ...
    fn alloc_from(&self, size: usize, align: usize) -> Result<usize, ()> {
        ...
        if excess_size > 0 && excess_size < core::mem::size_of::<Self>() {
            return Err(());
        }
...
impl LinkedListAllocator {
    ...
    fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        ...
        while let Some(ref mut region) = current.next {
            if let Ok(alloc_start) = region.alloc_from(size, align) {
